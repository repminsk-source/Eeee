...
class PlayerStats:
    ...
    def set_position(self, user_id, position, name=None):
        uid = str(user_id)
    ...
    def ensure_player(self, user_id, name=None):
        uid = str(user_id)
        if uid not in self.data:
            self.data[uid] = {
                "goals": 0, "assists": 0, "matches": 0,
        ...
        if name:
            self.data[uid]["name"] = name
        save_data(PLAYERS_FILE, self.data)
    def add_stat(self, user_id, stat, value=1, name=None):
        uid = str(user_id)
        self.ensure_player(user_id, name)
        if stat not in self.data[uid]:
            self.data[uid][stat] = 0
...
class ActivitySystem:
    def __init__(self):
        self.data = load_data(ACTIVITY_FILE, {})
    def update(self, user_id, chat_id):
        key = f"{chat_id}:{user_id}"
        self.data[key] = {
            "last_active": datetime.now().isoformat(),
            "warns_given": self.data.get(key, {}).get("warns_given", 0)
        }
        save_data(ACTIVITY_FILE, self.data)
    def get_inactive(self, chat_id, days=1):
        now = datetime.now()
        inactive = []
        for key, data in self.data.items():
            if key.startswith(f"{chat_id}:"):
                last_active = datetime.fromisoformat(data["last_active"])
                if now - last_active >= timedelta(days=days):
                    user_id = int(key.split(":", 1)[1])
                    inactive.append({
                        "user_id": user_id,
                        "last_active": last_active,
                        "days_inactive": (now - last_active).days,
                        "warns_given": data.get("warns_given", 0)
                    })
        return sorted(inactive, key=lambda x: x["days_inactive"], reverse=True)
    def add_warn(self, user_id, chat_id):
        key = f"{chat_id}:{user_id}"
        if key not in self.data:
            self.data[key] = {"last_active": datetime.now().isoformat(), "warns_given": 0}
        self.data[key]["warns_given"] = self.data[key].get("warns_given", 0) + 1
        save_data(ACTIVITY_FILE, self.data)
        return self.data[key]["warns_given"]
    def reset(self, user_id, chat_id):
        key = f"{chat_id}:{user_id}"
        if key in self.data:
            del self.data[key]
...
class AntiRaidSystem:
    def __init__(self):
        self.data = load_data(RAID_FILE, {
            "enabled": {},
            "join_tracker": {},
            "auto_enabled": {},
            "settings": {"join_window": 60, "max_joins": 5, "auto_enable_threshold": 5}
        })
    def is_enabled(self, chat_id):
        return self.data.get("enabled", {}).get(str(chat_id), False) or self.data.get("auto_enabled", {}).get(str(chat_id), False)
    def is_manually_enabled(self, chat_id):
        return self.data.get("enabled", {}).get(str(chat_id), False)
    def toggle(self, chat_id):
        cid = str(chat_id)
        if cid not in self.data["enabled"]:
            self.data["enabled"][cid] = False
        self.data["enabled"][cid] = not self.data["enabled"][cid]
        # Если ручно выключаем — сбрасываем авто-включение тоже
        if not self.data["enabled"][cid]:
            self.data["auto_enabled"][cid] = False
        save_data(RAID_FILE, self.data)
        return self.data["enabled"][cid]
    def disable_auto(self, chat_id):
        cid = str(chat_id)
        self.data["auto_enabled"][cid] = False
        save_data(RAID_FILE, self.data)
    def check_join(self, chat_id, user_id):
        cid = str(chat_id)
        now = datetime.now().timestamp()
        if cid not in self.data["join_tracker"]:
            self.data["join_tracker"][cid] = []
        window = self.data["settings"]["join_window"]
        self.data["join_tracker"][cid] = [t for t in self.data["join_tracker"][cid] if now - t < window]
        self.data["join_tracker"][cid].append(now)
        save_data(RAID_FILE, self.data)
        joins = len(self.data["join_tracker"][cid])
        max_joins = self.data["settings"]["max_joins"]
        auto_threshold = self.data["settings"].get("auto_enable_threshold", 3)

        # Авто-включение анти-рейда при подозрительной активности
        if not self.is_enabled(chat_id) and joins >= auto_threshold:
            self.data["auto_enabled"][cid] = True
            save_data(RAID_FILE, self.data)
            return True, f"🚨 <b>АВТО-РЕЙД ЗАЩИТА АКТИВИРОВАНА!</b>\n\nОбнаружено {joins} входов за {window} сек!\nАнти-рейд включён автоматически."

        if self.is_enabled(chat_id) and joins > max_joins:
            return True, f"🚨 <b>РЕЙД ОБНАРУЖЕН!</b>\n\n{joins} входов за {window} секунд!\nНовичок забанен."
        return False, None
...
async def inactivity_task():
    while True:
        try:
            ...
            now = datetime.now()
            chats = {}
            for key in list(activity.data.keys()):
                if ":" in key:
                    chat_id_str, user_id_str = key.split(":", 1)
                    chat_id = int(chat_id_str)
                    user_id = int(user_id_str)
                    if chat_id not in chats:
                        chats[chat_id] = []
                    chats[chat_id].append(user_id)
            for chat_id in chats:
                inactive_players = activity.get_inactive(chat_id, days=1)
                for player in inactive_players:
                    user_id = player["user_id"]
                    if is_admin(user_id, chat_id):
                        continue
                    warns_count = player["warns_given"]
                    if warns_count >= 2:
                        count = warns.add(user_id, chat_id, "Неактивность более 1 дня (3/3)", 0)
                        try:
                            await bot.ban_chat_member(chat_id, user_id)
                            bans.add(user_id, chat_id, None, "3 варна за неактивность", 0)
                            await bot.send_message(
                                chat_id,
                                f"🚫 <b>Игрок #{str(user_id)[-4:]}</b> забанен за неактивность (3/3 варна)",
                                parse_mode="HTML"
                            )
                        except:
                            pass
                        activity.reset(user_id, chat_id)
                    else:
                        count = warns.add(user_id, chat_id, f"Неактивность {player['days_inactive']} дней", 0)
                        activity.add_warn(user_id, chat_id)
                        try:
                            await bot.send_message(
                                chat_id,
                                f"⚠️ <b>Варн #{count}</b> за неактивность — Игрок #{str(user_id)[-4:]}\n"
                                f"📋 Неактивен: {player['days_inactive']} дней\n"
                                f"🚫 <b>{3 - count} варна до бана!</b>",
                                parse_mode="HTML"
                            )
                        except:
                            pass
        except Exception as e:
            print(f"Ошибка в inactivity_task: {e}")
...
async def on_chat_member_update(event: ChatMemberUpdated):
    ...
    if event.new_chat_member.status == "member" and event.old_chat_member.status in ["left", "kicked"]:
        is_raid, raid_msg = anti_raid.check_join(chat.id, user.id)
        if is_raid:
            try:
                await bot.send_message(chat.id, raid_msg, parse_mode="HTML")
                await bot.ban_chat_member(chat.id, user.id)
                bans.add(user.id, chat.id, None, "Анти-рейд: подозрительный вход", 0)
                return
            except:
                pass
...
async def pos_gk(callback: CallbackQuery):
    players.set_position(callback.from_user.id, "gk", callback.from_user.first_name)
...
for pos_key, pos_name in POSITIONS.items():
    ...
    async def pos_handler(callback: CallbackQuery, pos=pos_key, name=pos_name):
        players.set_position(callback.from_user.id, pos, callback.from_user.first_name)
        await callback.message.edit_text(
...
async def cmd_top(message: Message):
    ...
    stat_map = {
        "goals": ("⚽ Голы", "голов"),
        "assists": ("🎯 Передачи", "передач"),
        "wins": ("✅ Победы", "побед"),
        "matches": ("🏟️ Матчи", "матчей"),
        "yellow_cards": ("🟨 Жёлтые карточки", "ЖК"),
        "red_cards": ("🟥 Красные карточки", "КК"),
    }
    stat_key = args[1].lower() if len(args) > 1 else "goals"
    if stat_key not in stat_map:
        await message.answer(
            "❌ Доступные категории:\n"
            "<code>goals</code> · <code>assists</code> · <code>wins</code> · "
            "<code>matches</code> · <code>yellow_cards</code> · <code>red_cards</code>",
            parse_mode="HTML"
        )
        return
    stat_label, stat_unit = stat_map[stat_key]
    sorted_players = sorted(
        [(uid, data) for uid, data in all_players.items() if data.get("position")],
        key=lambda x: x[1].get(stat_key, 0), reverse=True
    )[:10]
    text = f"🏆 <b>Топ — {stat_label} — Aurum FC</b>\n\n"
    for i, (uid, data) in enumerate(sorted_players, 1):
        pos = POSITIONS.get(data.get("position", ""), "")
        name = data.get("name", f"Игрок #{uid[:4]}")
        medal = {1: "🥇", 2: "🥈", 3: "🥉"}.get(i, f"{i}.")
        text += f"{medal} <b>{name}</b> {pos} — {data.get(stat_key, 0)} {stat_unit}\n"
    text += f"\n<i>Категории: /top · /top assists · /top wins · /top matches</i>"
    await message.answer(text, parse_mode="HTML")

@router.message(Command("compare"))
async def cmd_compare(message: Message):
    if not message.reply_to_message:
        await message.answer("❌ Ответь на сообщение игрока которого хочешь сравнить с собой.")
        return
    uid1 = str(message.from_user.id)
    uid2 = str(message.reply_to_message.from_user.id)
    s1 = players.get(message.from_user.id)
    s2 = players.get(message.reply_to_message.from_user.id)
    if not s1 or not s1.get("position"):
        await message.answer("❌ Ты ещё не зарегистрирован. Используй /start.")
        return
    if not s2 or not s2.get("position"):
        await message.answer("❌ Этот игрок ещё не зарегистрирован.")
        return
    n1 = message.from_user.first_name
    n2 = message.reply_to_message.from_user.first_name
    def w(v1, v2):
        if v1 > v2: return "✅", "➖"
        elif v1 < v2: return "➖", "✅"
        return "🔸", "🔸"
    text = f"⚖️ <b>Сравнение игроков</b>\n\n"
    text += f"<b>{n1}</b> vs <b>{n2}</b>\n\n"
    stats = [("⚽ Голы", "goals"), ("🎯 Передачи", "assists"),
             ("🏟️ Матчи", "matches"), ("✅ Победы", "wins"),
             ("🤝 Ничьи", "draws"), ("❌ Поражения", "losses"),
             ("🟨 ЖК", "yellow_cards"), ("🟥 КК", "red_cards")]
    for label, key in stats:
        v1, v2 = s1.get(key, 0), s2.get(key, 0)
        e1, e2 = w(v1, v2)
        text += f"{label}: {e1} <b>{v1}</b> — <b>{v2}</b> {e2}\n"
    await message.answer(text, parse_mode="HTML")
...
<i>Бот напомнит за 24 часа и за 1 час до события!</i>"""
    await message.answer(text, parse_mode="HTML")
    attend_kb = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="✅ Приду", callback_data=f"attend_yes_{event_id}"),
            InlineKeyboardButton(text="❌ Не приду", callback_data=f"attend_no_{event_id}"),
            InlineKeyboardButton(text="🤔 Не знаю", callback_data=f"attend_maybe_{event_id}"),
        ]
    ])
    await message.answer(
        f"📋 <b>{etype_info['emoji']} {title}</b>\n📅 {format_datetime(event_dt)}\n\nОтметь своё присутствие:",
        reply_markup=attend_kb, parse_mode="HTML"
    )
...
async def cmd_addstat(message: Message):
    ...
    try:
        ...
        return
    target = message.reply_to_message.from_user
    players.add_stat(target.id, stat_type, value, target.first_name)
...
async def cmd_result(message: Message):
    ...
    if not message.reply_to_message:
        await message.answer(
            ...
            "Формат: <code>/result win</code> · <code>/result draw</code> · <code>/result loss</code>",
            parse_mode="HTML"
        )
        return
    args = message.text.split()
    if len(args) < 2:
        await message.answer(
            "❌ Укажи результат:\n"
            "<code>/result win</code> — победа\n"
            "<code>/result draw</code> — ничья\n"
            "<code>/result loss</code> — поражение",
            parse_mode="HTML"
        )
        return
    result = args[1].lower()
    result_map = {
        "win": ("wins", "✅ Победа"), "w": ("wins", "✅ Победа"),
        "победа": ("wins", "✅ Победа"), "поб": ("wins", "✅ Победа"),
        "draw": ("draws", "🤝 Ничья"), "d": ("draws", "🤝 Ничья"),
        "ничья": ("draws", "🤝 Ничья"), "ничь": ("draws", "🤝 Ничья"),
        "loss": ("losses", "❌ Поражение"), "l": ("losses", "❌ Поражение"),
        "поражение": ("losses", "❌ Поражение"), "пор": ("losses", "❌ Поражение"),
    }
    if result not in result_map:
        await message.answer("❌ Используй: win · draw · loss (или: победа · ничья · поражение)")
        return
    stat_key, stat_label = result_map[result]
    target = message.reply_to_message.from_user
    players.add_stat(target.id, "matches", 1, target.first_name)
    players.add_stat(target.id, stat_key, 1, target.first_name)
    p = players.get(target.id)
    await message.answer(
        f"✅ Записано для {get_mention(target)}:\n"
        f"{stat_label} | Матчей: {p.get('matches', 0)} | "
        f"В: {p.get('wins', 0)} Н: {p.get('draws', 0)} П: {p.get('losses', 0)}",
        parse_mode="HTML"
    )
...
async def cmd_antiraid(message: Message):
    ...
    if message.chat.type == ChatType.PRIVATE:
        ...
        return
    args = message.text.split()
    if len(args) > 1 and args[1].lower() in ["off", "выкл", "stop"]:
        anti_raid.data["enabled"][str(message.chat.id)] = False
        anti_raid.data["auto_enabled"][str(message.chat.id)] = False
        save_data(RAID_FILE, anti_raid.data)
        await message.answer(
            "🔴 <b>Анти-рейд ВЫКЛЮЧЕН</b>\n\n"
            "Защита отключена полностью (включая авто-активацию).",
            parse_mode="HTML"
        )
        return
    enabled = anti_raid.toggle(message.chat.id)
    manual = anti_raid.is_manually_enabled(message.chat.id)
    auto = anti_raid.data.get("auto_enabled", {}).get(str(message.chat.id), False)
    if manual:
        status = "🟢 ВКЛЮЧЕНА (ручно)"
    elif auto:
        status = "🟡 ВКЛЮЧЕНА (авто)"
    else:
        status = "🔴 ВЫКЛЮЧЕНА"
    await message.answer(
        f"🛡️ <b>Анти-рейд защита</b>\n\n"
        f"Статус: {status}\n\n"
        f"📊 Настройки:\n"
        f"• Лимит входов: {anti_raid.data['settings']['max_joins']} за {anti_raid.data['settings']['join_window']} сек\n"
        f"• Авто-активация при: {anti_raid.data['settings'].get('auto_enable_threshold', 5)} входов\n\n"
        f"<i>При превышении лимита — автобан новичков</i>\n"
        f"<code>/antiraid off</code> — полное отключение",
        parse_mode="HTML"
    )
...
async def cmd_inactive(message: Message):
    ...
    if message.chat.type == ChatType.PRIVATE:
        await message.answer("❌ Только в группах.")
        return
    inactive_players = activity.get_inactive(message.chat.id, days=1)
    if not inactive_players:
        await message.answer("✅ Нет неактивных игроков (более 1 дня).")
        return
    text = "😴 <b>Неактивные игроки</b>\n\n"
    for p in inactive_players[:20]:
        warns_count = p["warns_given"]
        status = "🟢" if warns_count == 0 else "🟡" if warns_count == 1 else "🔴"
        text += f"{status} <b>ID: {p['user_id']}</b> — {p['days_inactive']} дней неактивен"
        if warns_count > 0:
            text += f" (варнов за неактивность: {warns_count}/2)"
        text += "\n"
    text += "\n<i>1 варн = предупреждение, 2 варна = бан</i>"
    await message.answer(text, parse_mode="HTML")
...
/mute 60 [причина] — Мут на 60 минут
/unmute — Размутить
/ban 1440 [причина] — Бан (без числа = навсегда)
/unban — Разбанить
/kick [причина] — Кик
...
/playernotes (ответом) — Заметки об игроке
/mynotes — Мои заметки
/delnote 1 — Удалить заметку №1

<b>📅 Расписание:</b>
/schedule — Показать расписание
/addschedule Название | дата | описание — Добавить
/delschedule 1 — Удалить событие №1

<b>📊 Голосования:</b>
/poll Вопрос / вар1 / вар2 / вар3 — Создать опрос
/polllist — Активные опросы
/delpoll 1 — Удалить опрос №1

<b>📊 Статистика (админы):</b>
/addstat goals 2 — Добавить 2 гола (ответом)
/setstat matches 10 — Установить значение
/result win — Записать победу (ответом)
/result draw — Ничья | /result loss — Поражение
/playerstats — Статистика игрока
/compare — Сравнить двух игроков (ответом)
/top — Топ по голам
/top assists — Топ по передачам
/top wins — Топ по победам

<b>👮 Управление админами:</b>
/admins — Список админов
/addadmin 123456 — Добавить тренера/админа
/removeadmin 123456 — Удалить админа

<b>🛡️ Анти-рейд:</b>
/antiraid — Вкл/выкл защиту от рейда

<b>😴 Неактивность:</b>
/inactive — Список неактивных игроков
...
async def on_attendance(callback: CallbackQuery):
    parts = callback.data.split("_")
    status = parts[1]
    ...
    if key not in attendance_data:
        attendance_data[key] = {"yes": [], "no": [], "maybe": []}
    for s in ["yes", "no", "maybe"]:
        if user_id in attendance_data[key][s]:
            attendance_data[key][s].remove(user_id)
    attendance_data[key][status].append(user_id)
    yes = len(attendance_data[key]["yes"])
    no = len(attendance_data[key]["no"])
    maybe = len(attendance_data[key]["maybe"])
    status_text = {"yes": "✅ Приду", "no": "❌ Не приду", "maybe": "🤔 Не знаю"}[status]
    await callback.answer(f"{status_text} — записано!")
    try:
        await callback.message.edit_reply_markup(reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [
                InlineKeyboardButton(text=f"✅ Приду ({yes})", callback_data=f"attend_yes_{event_id}"),
                InlineKeyboardButton(text=f"❌ Не приду ({no})", callback_data=f"attend_no_{event_id}"),
                InlineKeyboardButton(text=f"🤔 Не знаю ({maybe})", callback_data=f"attend_maybe_{event_id}"),
            ]
        ]))
    except:
        pass
...
# ═══════════════════════════════════════════════════════════════
#  АНТИСПАМ (МЯГКИЙ + ЗАЩИТА АДМИНОВ)
# ═══════════════════════════════════════════════════════════════
...
FLOOD_LIMIT = 8       # макс сообщений
FLOOD_WINDOW = 10     # за N секунд
AUTO_MUTE_AFTER = 5   # предупреждений до авто-мута
AUTO_MUTE_MINS = 10   # минут мута

def get_content_key(msg: Message):
    if msg.text:
        return msg.text
    if msg.sticker:
        return f"sticker:{msg.sticker.file_unique_id}"
    if msg.animation:
        return f"gif:{msg.animation.file_unique_id}"
    if msg.video:
        return f"video:{msg.video.file_unique_id}"
    return None

async def cleanup_spam_trackers():
    while True:
        await asyncio.sleep(300)
        now = datetime.now().timestamp()
        async with spam_lock:
            for k in list(spam_tracker.keys()):
                if now - spam_tracker[k]["time"] > 120:
                    del spam_tracker[k]
            for k in list(flood_tracker.keys()):
                flood_tracker[k] = [t for t in flood_tracker[k] if now - t < FLOOD_WINDOW]
                if not flood_tracker[k]:
                    del flood_tracker[k]
            for k in list(offense_tracker.keys()):
                if offense_tracker[k] == 0:
                    del offense_tracker[k]
...
async def check_mute(message: Message):
    ...
    activity.update(user_id, chat_id)
    ...
    async with spam_lock:
        ...
        flood_tracker[user_key] = [t for t in flood_tracker[user_key] if now_ts - t < FLOOD_WINDOW]
        flood_tracker[user_key].append(now_ts)
        if len(flood_tracker[user_key]) > FLOOD_LIMIT:
            is_spam = True
            spam_type = "flood"

        # --- Дубли ---
        if not is_spam:
            content_key = get_content_key(message)
            if content_key:
                dup_key = f"{user_key}:{content_key}"
                entry = spam_tracker.get(dup_key)
                if entry and now_ts - entry["time"] < 45:
                    is_spam = True
                    spam_type = "duplicate"
                    entry["count"] = entry.get("count", 1) + 1
                else:
                    spam_tracker[dup_key] = {"time": now_ts, "count": 1}
    ...
    if is_spam:
        ...
        offense_tracker[user_key] = offense_tracker.get(user_key, 0) + 1
        offenses = offense_tracker[user_key]

        if offenses >= AUTO_MUTE_AFTER:
            offense_tracker[user_key] = 0
            until = mutes.add(user_id, chat_id, AUTO_MUTE_MINS, "Антиспам", 0)
            try:
                from aiogram.types import ChatPermissions
            ...
            notif = await message.answer(
                f"🔇 {get_mention(message.from_user)} получает мут на {AUTO_MUTE_MINS} мин за систематический спам!",
                parse_mode="HTML"
            )
        else:
            if spam_type == "flood":
                warn_text = (
                    f"⚡ {get_mention(message.from_user)}, притормози! "
                    f"Слишком много сообщений. [{offenses}/{AUTO_MUTE_AFTER} — потом мут]"
                )
            else:
                warn_text = (
                    f"🚫 {get_mention(message.from_user)}, не спамь одинаковым! "
                    f"[{offenses}/{AUTO_MUTE_AFTER} — потом мут]"
                )
            notif = await message.answer(warn_text, parse_mode="HTML")
...
async def main():
    print("⚽ Aurum FC Bot v3.1 запущен!")
    asyncio.create_task(reminder_task())
    asyncio.create_task(cleanup_spam_trackers())
    asyncio.create_task(inactivity_task())
