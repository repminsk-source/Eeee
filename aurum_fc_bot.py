...
async def check_mute(message: Message):
    ...
    if message.text:
        spam_key = f"{message.chat.id}:{message.from_user.id}"
        last = spam_tracker.get(spam_key)
        now_ts = datetime.now().timestamp()
        if last and last["text"] == message.text and now_ts - last["time"] < 30:
            try:
            ...
            try:
                ...
                pass
            return
        spam_tracker[spam_key] = {"text": message.text, "time": now_ts}
