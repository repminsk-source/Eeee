...
spam_tracker = {}    # дубли: "chat:user:content" -> {time, count}
flood_tracker = {}   # флуд: "chat:user" -> [timestamps]
offense_tracker = {} # нарушения: "chat:user" -> count
spam_lock = asyncio.Lock()

FLOOD_LIMIT = 5       # макс сообщений
FLOOD_WINDOW = 5      # за N секунд
AUTO_MUTE_AFTER = 3   # предупреждений до авто-мута
    ...
    async with spam_lock:
        ...
        if not is_spam:
            ...
            if content_key:
                ...
                if entry and now_ts - entry["time"] < 30:
                    is_spam = True
                    spam_type = "duplicate"
