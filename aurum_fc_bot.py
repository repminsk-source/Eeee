...
async def check_mute(message: Message):
    ...
    if content_key:
        spam_key = f"{message.chat.id}:{message.from_user.id}:{content_key}"
        now_ts = datetime.now().timestamp()

        async with spam_lock:
            entry = spam_tracker.get(spam_key)
            is_spam = entry and now_ts - entry["time"] < 30
            if is_spam:
                entry["count"] = entry.get("count", 1) + 1
            else:
                spam_tracker[spam_key] = {"time": now_ts, "count": 1, "warned": False}
        ...
        if is_spam:
            ...
            if not entry.get("warned"):
                ...
                notif = await message.answer(
                    f"🚫 {get_mention(message.from_user)}, не спамь одинаковыми сообщениями!",
                    parse_mode="HTML"
                )
